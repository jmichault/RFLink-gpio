...
boolean Plugin_040(byte function, char *string) {
      ...
      byte data[7];
      byte channel=0;
      if(!RawSignal.Multiply) RawSignal.Multiply=1;
      //==================================================================================
      // get all 28 bits
      for(byte x=2;x <=MEBUS_PULSECOUNT-2;x+=2) {
         if (RawSignal.Pulses[x+1]*RawSignal.Multiply > 700) return false; // make sure inbetween pulses are not too long
         if (RawSignal.Pulses[x]*RawSignal.Multiply > 3000) {
            bitstream = (bitstream << 1) | 0x1; 
         } else {
            if (RawSignal.Pulses[x]*RawSignal.Multiply > 2500) return false; // invalid pulse length
            if (RawSignal.Pulses[x]*RawSignal.Multiply < 1300) return false; // invalid pulse length
