 ...
 * 00010010 01110100 01111000   = 0x 12 74 78
 \*********************************************************************************************/
#define ALARMPIRV3_PULSECOUNT 50

#define ALARMPIRV3_PULSEMID  700/RAWSIGNAL_SAMPLE_RATE
#define ALARMPIRV3_PULSEMAX  2000/RAWSIGNAL_SAMPLE_RATE
#define ALARMPIRV3_PULSEMIN  150/RAWSIGNAL_SAMPLE_RATE

#ifdef PLUGIN_062
boolean Plugin_062(byte function, char *string) {
      if (RawSignal.Number != ALARMPIRV3_PULSECOUNT) return false;
      if (RawSignal.Pulses[0]==63) return false;    // No need to test, packet for plugin 63
      unsigned long bitstream=0L;
      //==================================================================================
      for(byte x=2;x<=48;x=x+2) {
        if (RawSignal.Pulses[x] > ALARMPIRV3_PULSEMID) {
           if (RawSignal.Pulses[x] > ALARMPIRV3_PULSEMAX) return false;   // pulse too long
           if (RawSignal.Pulses[x-1] > ALARMPIRV3_PULSEMID) return false; // invalid pulse sequence 10/01
           bitstream = (bitstream << 1) | 0x1; 
        } else { 
           if (RawSignal.Pulses[x] < ALARMPIRV3_PULSEMIN) return false;   // pulse too short
           if (RawSignal.Pulses[x-1] < ALARMPIRV3_PULSEMID) return false; // invalid pulse sequence 10/01
